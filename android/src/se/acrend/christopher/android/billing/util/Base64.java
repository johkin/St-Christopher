...
public class Base64 {
   ...
   * not check to make sure your arrays are large enough to accommodate
   ...
   *          The data to convert
   * @param doPadding
   *          is {@code true} to pad result with '=' chars if it does not fall
  ...
  /**
   * Decodes web safe Base64 content in byte array format and returns the
   * decoded byte array. Web safe encoding uses '-' instead of '+', '_' instead
   * of '/'
   * 
   * @param source
   *          the Base64 encoded data
  ...
  /**
   * Decodes Base64 content using the supplied decodabet and returns the decoded
   * byte array.
   * 
   * @param source
   *          the Base64 encoded data
        ...
        if (sbiDecode >= EQUALS_SIGN_ENC) {
          ...
          if (sbiCrop == EQUALS_SIGN) {
            ...
            if ((b4Posn == 0) || (b4Posn == 1)) {
              ...
              throw new Base64DecoderException("padding byte '=' falsely signals end of encoded value " + "at offset "
                  + i);
