...
import se.acrend.sj2cal.content.ProviderTypes.Columns;
...
public class ProviderHelper {
  ...
  private List<DbModel> query(final Uri uri, final String selection, final String[] selectionArgs) {
    String[] projection = { BaseColumns._ID, ProviderTypes.Columns.ORIGINAL_ARRIVAL,
        ...
        ProviderTypes.Columns.TRAIN_NO, Columns.TICKET_CODE, Columns.TICKET_TEXT, ProviderTypes.Columns.REGISTERED,
  ...
  public DbModel findByCode(final String code) {
    List<DbModel> result = query(ProviderTypes.CONTENT_URI, Columns.TICKET_CODE + " = ?", new String[] { code });
    if (result.isEmpty()) {
      return null;
    }
    return result.get(0);
