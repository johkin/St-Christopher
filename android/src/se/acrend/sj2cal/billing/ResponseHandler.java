...
package se.acrend.sj2cal.billing;

import se.acrend.sj2cal.billing.Consts.PurchaseState;
import se.acrend.sj2cal.billing.Consts.ResponseCode;
import se.acrend.sj2cal.receiver.BillingReceiver;
import se.acrend.sj2cal.service.BillingService;
import se.acrend.sj2cal.service.BillingService.RequestPurchase;
 ...
 * not do that. You should modify and obfuscate this code before using it.
 */
public class ResponseHandler {
  private static final String TAG = "ResponseHandler";

  /**
   * This is a static instance of {@link PurchaseObserver} that the application
   * creates and registers with this class. The PurchaseObserver is used for
   * updating the UI if the UI is visible.
   */
   ...
   */
  public static synchronized void register(final PurchaseObserver observer) {
   ...
   */
  public static synchronized void unregister(final PurchaseObserver observer) {
   ...
   */
  public static void checkBillingSupportedResponse(final boolean supported) {
   ...
   * @param intent
   *          an intent containing a request id in an extra field that will be
   *          passed to the buy page activity when it is created
   */
   ...
   * called is if the user bought something on another device running this same
   * app. Then Android Market notifies the other devices that the user has
   * purchased an item, in which case the BillingService will also call this
   * method. Finally, this method can be called if the item was refunded.
   * 
   * @param purchaseState
    ...
    new Thread(new Runnable() {
      ...
      public void run() {
        ...
        synchronized (ResponseHandler.class) {
          if (sPurchaseObserver != null) {
            sPurchaseObserver.postPurchaseStateChange(purchaseState, productId, purchaseTime, developerPayload);
          }
        }
      }
    }).start();
   ...
   */
  public static void responseCodeReceived(final Context context, final RequestPurchase request,
