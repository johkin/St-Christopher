...
import se.acrend.sj2cal.billing.Consts;
import se.acrend.sj2cal.billing.Consts.ResponseCode;
import se.acrend.sj2cal.billing.ResponseHandler;
 ...
 * {@link #onReceive(Context, Intent)} returns. You should modify and obfuscate
 * this code before using it.
 */
public class BillingReceiver extends RoboBroadcastReceiver {
   ...
   * change. The signedData parameter is a plaintext JSON string that is signed
   * by the server with the developer's private key. The signature for the
  ...
  /**
   * This is called when Android Market sends a "notify" message indicating that
   * transaction information is available. The request includes a nonce (random
   * number used once) that we generate and Android Market signs and sends back
   * to us with the purchase state and other transaction details. This
   * BroadcastReceiver cannot bind to the MarketBillingService directly so it
   * starts the {@link BillingService}, which does the actual work of sending
   * the message.
   * 
   * @param context
   *          the context
